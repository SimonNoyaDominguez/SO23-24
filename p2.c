...
static void print_memory_block(char *pos, int bytes, List memoryList);
...
memory_item *search_mem_pos(List memList, void *memdir){
	Lpos auxPos;
...
static void print_memory_block(char *pos, int bytes, List memList){
	...
	if(addr!=NULL && (infoData=search_mem_pos(memList, addr))!=NULL){
		if(bytes>0){
	...
	if(lenArg==1)
		print_memList(SHARED_MEM);
        	...
        	if (lenArg == 2) {
				...
				infoData->data.file_name=NULL;
				infoData->data.file_descriptor=0;
				free(infoData->data.file_name);  // Make sure to free any dynamically allocated data
                		free(infoData);
            		} else {
               		 	printf("No se encontró un bloque de memoria con el tamaño especificado.\n");
            			}
		...
		// Se reserva un bloque de memoria compartida y se añade a la lista de memoria compartida
			...
			if((nwItem->data.key = (key_t)strtoul(args[1], NULL, 10))==0 || ObtenerMemoriaShmget(nwItem->data.key, nwItem->size, nwItem)==NULL){
...
int unmapped_memory_block(const char *file_name) {
    Lpos auxPos;
    memory_item *infoData=NULL;
...
int cmd_Mmap(const int lenArg, char *args[COMMAND_LEN]) {
    ...
    if (lenArg == 1)
        ...
        if (lenArg <= 2) {
           ...
           for (auxPos = firstElement(memList); auxPos != NULL; auxPos = nextElement(memList, auxPos)) {
                memory_item *infoData = getElement(memList, auxPos);
                if (infoData->type == MAPPED_MEM) {
                    // Llamamos a la función para desmapear la memoria
                    int result = unmapped_memory_block(infoData->data.file_name);

                    if (result == 1) {
                        printf("Desmapeado correctamente el archivo\n");
                    } else {
                        printf("Error al desmapear el archivo\n");
                    }
